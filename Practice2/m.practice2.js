var $P2_table = document.getElementById("Table");
var $P2_tr = document.getElementsByClassName("TR"); //P2_tr:所有的单元格ClassName
var $P2_input = document.getElementsByTagName("input");// P2_input:指向文本输入框
var $ti_shi = document.getElementById("Hint");//提示输入框
var $P2_button = document.getElementById("addButton");//新增按钮

//给单元格添加鼠标监听事件：
for (var i = 1; i < $P2_tr.length; i++) {
	$P2_tr[i].addEventListener("mouseover", function () {
		this.style.background = "#eee";
	});
	$P2_tr[i].addEventListener("mouseout", function () {
...
for (var j = 0; j < $P2_input.length; j++) {
	$P2_input[j].addEventListener("mouseover", function () {
		this.style.border = "solid 1px red";
	});
	$P2_input[j].addEventListener("mouseout", function () {
...
$P2_button.onclick = function () {
	...
	New_tr.className = "TR";//给tr赋值class
	...
	New_td1.setAttribute("class", "TD XHNumber");//学号单元格添加class属性
	...
	New_td2.className = "nameTD";//给姓名单元格赋值class属性
	...
	New_td3.className = "TD";//删除单元格的class属性
	...
	if (New_XH.length !== 5/*学号的长度不为5*/ || New_Name.length > 3/*姓名的长度大于3*/ || New_Name.length < 2/*姓名的长度小于2*/) {
		$ti_shi.innerHTML = "添加失败，学号的长度为 5; 姓名的长度 2-3";//提示报错
	}
	else {
		...
		if (P2_XuehaoArray[0].toUpperCase() !== "X") {                                 /*判断学号是否合法*/
			...
			for (var P2_tmpi = 0; P2_tmpi < document.getElementsByClassName("TD XHNumber").length; P2_tmpi++) {
				if (New_XH.toUpperCase() === document.getElementsByClassName("TD XHNumber")[P2_tmpi].innerHTML) {
					$ti_shi.innerHTML = "添加失败，学号不能重复";
					P2_XHrepeat = false;
					break;
				}
			}
			for (var P2_tmpi2 = 0; P2_tmpi2 < New_Name.length; P2_tmpi2++) {
				if (!/^[\u4e00-\u9fa5],{0,}$/.test(P2_NameArray[P2_tmpi2]) && !/^[A-Za-z]+$/.test(P2_NameArray[P2_tmpi2])) {
					$ti_shi.innerHTML = "添加失败，姓名只能是英文或汉字";
					P2_XHrepeat = flase;
					break;
				}
